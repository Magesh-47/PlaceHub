...
public class StudentController {
    ...
    public ResponseEntity<StudentProfileDetailsResponse> updateEducation(
            @Valid @RequestBody EducationListRequest request,
            Authentication authentication) {
        return ResponseEntity.ok(studentService.updateEducation(authentication.getName(), request.getEducation()));
    }

    @PutMapping("/profile/experience")
    public ResponseEntity<StudentProfileDetailsResponse> updateExperience(
            @Valid @RequestBody ExperienceListRequest request,
            Authentication authentication) {
        return ResponseEntity.ok(studentService.updateExperience(authentication.getName(), request.getExperience()));
    }

    @PutMapping("/profile/skills")
    public ResponseEntity<StudentProfileDetailsResponse> updateSkills(
            @Valid @RequestBody SkillsUpdateRequest request,
            Authentication authentication) {
        return ResponseEntity.ok(studentService.updateSkills(authentication.getName(), request.getSkills()));
    }

    @PutMapping("/profile/certifications")
    public ResponseEntity<StudentProfileDetailsResponse> updateCertifications(
            @Valid @RequestBody CertificationListRequest request,
            Authentication authentication) {
